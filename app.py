...
def institutional_history(code, lookback=20):
    """取得法人歷史；並行抓取交易日，避免逐日等待造成長時間卡住。"""
    from concurrent.futures import ThreadPoolExecutor, as_completed

    dates = latest_business_dates(lookback + 8)
    ...
    def fetch(d):
        ...
        if code not in day:
            return None
        x = day[code]
        return {'date': d, 'foreign': x['foreign'], 'trust': x['trust'],
                'dealer': x['dealer'], 'total': x['total']}
    ...
    with ThreadPoolExecutor(max_workers=6) as ex:
        ...
        for f in as_completed(futures):
            try:
                x=f.result()
                if x: frames.append(x)
            except Exception:
                pass

    if not frames:
        return pd.DataFrame()
    return pd.DataFrame(frames).sort_values('date', ascending=False).head(lookback).reset_index(drop=True)
...
def revenue_history(code, months=24):
    """抓取月營收；並行查詢月份，避免 24 次 API 串行等待。"""
    from concurrent.futures import ThreadPoolExecutor, as_completed
    today = datetime.now().replace(day=1)
    month_dates=[today - timedelta(days=31*m) for m in range(min(months, 36))]

    def fetch(dt):
        j=http_json(
            'https://www.twse.com.tw/rwd/zh/afterTrading/FMSRFK',
            params={'date':dt.strftime('%Y%m01'),'stockNo':code,'response':'json'},
            timeout=8
        )
        rows=[]
        if not isinstance(j,dict) or not j.get('data'):
        ...
        for row in j['data']:
            ...
            date_idx=next((i for i,x in enumerate(vals) if re.search(r'\d{3,4}/\d{1,2}',x)),0)
            date=pd.to_datetime(roc_to_ad(vals[date_idx]),errors='coerce')
            if pd.isna(date): continue
            nums=[num(x) for x in vals]
            candidates=[x for x in nums if not pd.isna(x)]
            if candidates: rows.append({'date':date,'revenue':candidates[0]})
        return rows

    frames=[]
    with ThreadPoolExecutor(max_workers=5) as ex:
        futures=[ex.submit(fetch,dt) for dt in month_dates]
        for f in as_completed(futures):
            try: frames.extend(f.result())
            except Exception: pass
    if not frames: return pd.DataFrame()
    return pd.DataFrame(frames).drop_duplicates('date').sort_values('date').tail(months)
...
def cached_or_live_radar():
    """首頁快速載入：有完整快取就顯示完整雷達；沒有快取就顯示輕量自動選股。

    絕不在首頁啟動時執行 live_radar()，避免 20 檔逐檔抓法人/營收造成長時間 Loading。
    GitHub Actions 每日建立 radar_cache.json 後，首頁會自動升級成完整法人＋營收雷達。
    """
    c = load_cache()
    if isinstance(c, dict) and c.get('results'):
        return pd.DataFrame(c['results']), c.get('generated_at', ''), True
    fallback = market_only_radar()
    return fallback, datetime.now().strftime('%Y-%m-%d %H:%M') if not fallback.empty else '', False
...
if st.button('🔄 更新完整雷達', type='secondary'):
    with st.spinner('正在更新完整法人＋營收＋技術雷達，首次可能需要一些時間…'):
        fresh = live_radar()
    if not fresh.empty:
        cache_df = fresh
        cache_is_full = True
        ...
        st.warning('完整雷達目前無法取得，先維持快速自動選股結果。')

if not cache_df.empty:
    if cache_time:
        st.caption(
            f'資料快取時間：{cache_time}｜' + ('完整法人＋營收雷達' if cache_is_full else '快速自動選股模式')
        )
